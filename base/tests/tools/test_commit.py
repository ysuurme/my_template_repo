...
from tools.commit import generate_commit_message


def test_generate_commit_message_returns_string() -> None:
    mock_response = MagicMock()
    mock_response.text = "feat(utils): add commit message generator"

    with patch("tools.commit.genai.Client") as mock_client_class:
...
def test_generate_commit_message_strips_whitespace() -> None:
    mock_response = MagicMock()
    mock_response.text = "  fix(config): correct env loading  "
